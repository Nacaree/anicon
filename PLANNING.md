# AniCon Backend - Planning Document
...
```
1. User fills form (email, password, username) in Next.js
2. Next.js calls supabase.auth.signUp({ email, password })
3. Supabase creates auth.users row, returns JWT token
4. Next.js calls Spring Boot: POST /api/profiles
   - Header: Authorization: Bearer <token>
   - Body: { username, displayName }
5. Spring Boot validates JWT, creates profiles row
```
