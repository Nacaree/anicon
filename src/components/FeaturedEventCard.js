export default function FeaturedEventCard({
  ...
  return (
    <div className="relative rounded-xl h-48 md:h-64 w-full md:w-[500px] lg:w-[600px] overflow-hidden group cursor-pointer flex-shrink-0">
      {/* Background Image */}
      <img
        src={imageUrl}
        alt={title}
      ...
      <div className="relative h-full flex flex-col justify-between p-6 text-white z-10">
        <div>
          {tagline && <p className="text-xs mb-2 opacity-90">{tagline}</p>}
          <h3 className="text-3xl md:text-4xl font-bold mb-2">{title}</h3>
          {subtitle && (
            <p className="text-2xl md:text-3xl font-light italic">{subtitle}</p>
          )}
          {date && <p className="text-sm mt-2">{date}</p>}
